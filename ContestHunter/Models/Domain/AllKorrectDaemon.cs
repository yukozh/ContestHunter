...
namespace ContestHunter.Models.Domain
{
    public class AllKorrectDaemon : Daemon
    {
        ...
        bool DealRecord(Guid recID,string Host,int Port)
        {
        ...
        bool DealHunt(Guid huntID,string Host,int Port)
        {
            using (var db = new CHDB())
            {
                ...
                Parallel.ForEach(tinf, (TestInfo inf) =>
                    {
                        try
                        {
                            DealHunt(inf.hunt,inf.host,inf.port);
                            DealRecord(inf.record,inf.host,inf.port);
                        }
                        catch { }
                    });
