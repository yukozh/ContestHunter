    ...
    public class AllKorrectDaemon : Daemon
    {
        ...
        {
            {
                ...
                {
                    {"compileargv",new string[]{"-O2","code.cpp"}},
            ...
            },
            {
                ...
                new Dictionary<string,string[]>()
                {
                    {"compileargv",new string[]{"-O2","code.c"}},
        ...
        bool DealHunt(CHDB db)
        {
            ...
            using (var tester = new NativeRunner(Host, Port))
            {
                ...
                tester.PutFile("checker", Encoding.UTF8.GetBytes(DataChecker));
                ...
                tester.MoveFile2File(commands[(Record.LanguageType)rec.RECORD1.PROBLEM1.DataCheckerLanguage]["exec"][0], "checker");
                result = tester.Execute("checker", new string[] { }, MemoryLimit, TimeLimit, 100 * 1024 * 1024, RestrictionLevel.Strict, HuntData);
                if (result.Type != ExecuteResultType.Success)
                {
                    rec.Status = (int)Hunt.StatusType.BadData;
                    return true;
                ...
                if (result.Type == ExecuteResultType.Success)
                {
                    ...
                    result = Compile(rec.RECORD1.PROBLEM1.Comparer, (Record.LanguageType)rec.RECORD1.PROBLEM1.ComparerLanguage, tester);
                    if (result.Type != ExecuteResultType.Success)
                    {
                        rec.Status = (int)Hunt.StatusType.OtherError;
                        return true;
